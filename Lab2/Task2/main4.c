...
void teacher_chld_handler(int sig){
    ...
    while ((pid = waitpid(-1, &status, WNOHANG)) > 0) {
        ...
        if (WIFEXITED(status)) {
            // Save result
            results[result_index].pid = pid;
            results[result_index].issues = WEXITSTATUS(status);
            result_index++;
        }
    }
}
...
int main(int argc, char** argv){
    ...
    results = malloc(sizeof(student_result_t) * num_students);
        ...
        if(pid == 0){
            ...
            for (int part = 1; part <= p; part++)
            {
                ...
                for (int step = 0; step < t; step++) {
                    ...
                    if(r < prob){
                        total_issues++;
                        printf("Student[%d,%d] has issue (%d) doing taks!\n",i,getpid(),total_issues);

                        struct timespec penalty = {0,50*10000000L};
    ...
    for(int i=0; i < result_index; i++) {
        printf("%3d | %10d | %d\n", i+1, results[i].pid, results[i].issues);
    }
    
    int total = 0;
    for(int i=0; i<result_index; i++) total += results[i].issues;
